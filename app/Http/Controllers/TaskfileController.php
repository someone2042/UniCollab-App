...
{
    //
    public function show(Group $group, Task $task, Taskfile $taskfile)
    {
        if (auth()->user()->id != $task->user_id && auth()->user()->id != $group->leader_id) {
            return Redirect::back()->with('error', 'you are not allowed to see this file');
        }
        // dd($taskfile);
        ...
        if (in_array($lang, $array)) {
            $path = storage_path('app/public/' . $taskfile->path);
