...
<body class="h-screen bg-back background">
    ...
    <div class="center">
        ...
        <div id="content1" class="content1 overflow-auto h-calc-screen2">
                ...
                @foreach ($files as $file)
                    ...
                    <div class="child-div">
                        <div class="g-child-div relative overflow-hidden"
                            style="background-image: url('/ico/{{ $icon }}.ico');">
                            <a href="/group/{{ $mainGroup->id }}/projects/{{ $file->id }}"
                                class="h-4/5 absolute w-full block mt-6"></a>
                            <select id="mySelect"
                                class="block bg-transparent outline-none border-transparent focus:border-transparent focus:ring-0">
                                <option value="">‎ ‎ ‎ ‎ ‎ </option>
                                @foreach ($file->versions as $version)
                                    <option value="option1"
                                        data-href="/group/{{ $mainGroup->id }}/projects/{{ $file->id }}/{{ $version->id }}">
                                        {{ $version->version }}</option>
                                @endforeach
                                @if (auth()->user()->id == $mainGroup->leader_id)
                                    <option value="delete"
                                        data-href="/group/{{ $mainGroup->id }}/projects/{{ $file->id }}">rm
                                    </option>
                                @endif
                            </select>
                            <a href="/storage/{{ $file->currentVersion()->path }}" class=" relative block w-4"
                                style=" bottom: -80px; right: -25px;" download>
                                <svg xmlns="http://www.w3.org/2000/svg" class="opacity-20 hover:opacity-100"
                                    shape-rendering="geometricPrecision" width="20px" hieght="20px"
                                    text-rendering="geometricPrecision" image-rendering="optimizeQuality"
                                    fill-rule="evenodd" clip-rule="evenodd" viewBox="0 0 512 499.93">
            ...
            <div
                class="mx-4 mt-1 bg-blue-500 absolute right-3 p-2 bottom-1 hover:scale-95 hover:bg-blue-400 cursor-pointer h-fit rounded-full">
                <a href="/group/{{ $mainGroup->id }}/projects/zip">
                    <svg width="35px" height="35px" viewBox="0 0 48 48" fill="none"
                        ...
                        <path
                            ...
                            stroke="#FFF" stroke-width="4" stroke-linecap="round" stroke-linejoin="round" />
                        <path d="M36 20C36 13.3726 30.6274 8 24 8C17.3726 8 12 13.3726 12 20" stroke="#FFF"
                            stroke-width="4" stroke-linecap="round" stroke-linejoin="round" />
                        <path d="M17.0654 30.119L23.9999 37.0764L31.1318 30" stroke="#FFF" stroke-width="4"
                            stroke-linecap="round" stroke-linejoin="round" />
                        <path d="M24 20V33.5382" stroke="#FFF" stroke-width="4" stroke-linecap="round"
                            stroke-linejoin="round" />
                    </svg>
                </a>
            </div>
            @if (auth()->user()->id == $mainGroup->leader_id)
                <form action="/group/{{ $mainGroup->id }}/projects" method="POST" id="upload_id"
                    enctype="multipart/form-data">
                    @csrf
                    <label for="file"
                        class="cursor-pointer sticky flex justify-end bottom-0 h-1/12 w-full z-40 bg-white shadow-2xl shadow-black ">
                        <div
                            class="mx-4 mt-1 bg-blue-500 hover:scale-95 hover:bg-blue-400 cursor-pointer absolute right-20 p-2 bottom-1 h-fit rounded-full">
                            <?xml version="1.0" ?>
                            <svg viewBox="0 0 256 256" xmlns="http://www.w3.org/2000/svg" height='35px'
                                width='35px'>
                                <rect fill="none" height="200" width="200" />
                                <line fill="none" stroke="#FFF" stroke-linecap="round" stroke-linejoin="round"
                                    stroke-width="35" x1="40" x2="216" y1="128"
                                    y2="128" />
                                <line fill="none" stroke="#FFF" stroke-linecap="round" stroke-linejoin="round"
                                    stroke-width="35" x1="128" x2="128" y1="40"
                                    y2="216" />
                            </svg>
                        </div>
